...
- Represent game, replay, and audio-driver state as transparent structs.
- Read plain state directly. Do not add getters or `edit_*` accessors around a
  field merely to make it private.
- Keep an operation as a method when it naturally acts on one value, such as
  `game.step()`, `board.set()`, or `replay.rewind()`.
- Prefer a free function when behavior is stateless or combines independent
  values without a clear owner.
- Reserve private classes and trailing-underscore members for actual resource
  ownership boundaries, such as SDL audio streams. Video texture handles may
  remain in transparent structs when free lifecycle functions make ownership
  obvious.
- Do not hide ordinary domain data in the name of hypothetical invariants.
  Keep mutation paths clear, deliberate, and easy to search instead.
