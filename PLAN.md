...
“Modern” describes the architecture and host rendering. It does not silently
replace the original rules with seven-bag randomization, SRS, hold, ghost
pieces, or guideline scoring. Those rules live in the separately identified,
opt-in Modern preset; Enhanced remains the default classic ruleset.
...
- no generic ECS for seven pieces and a small number of rendering actors;
...
deterministic pixel behavior; floating point belongs at host rendering
boundaries where exact simulation identity does not depend on it.

Domain state is deliberately transparent. Game, versus, replay, and
audio-driver state use public structs and direct field reads instead of getter
walls or `edit_*` escape hatches. Operations remain methods only when they
naturally act on one value; stateless rules remain free functions. Private
classes are reserved for concrete host-resource lifetimes such as SDL audio
streams and renderer textures, where unrestricted mutation could leak or
invalidate an external resource.
...
src/video/         layouts, indexed rendering, animation, effects
src/audio/         Tetris driver, effects, gb-audio state and SDL output bridge
src/storage/       high-score file persistence
src/settings.*     persistent user choices
src/frame.*        explicit host-frame phases
src/window.*       window setup, events and display requests
src/main.cpp       initialization, main loop and cleanup
...
`GameContent`, a `Rules` value, and explicit per-step commands. It publishes a
...
- `VersusMatch`: two boards, shared generation, attacks, wins and match progression;
- `GameState`: startup, title, menus, demo, play, pause, results, high scores and endings;
- `Replay`: rules identity, initial state, random samples and step commands;
- `GameEvents`: meaningful facts consumed by rendering and audio.
...
- metasprites and rendering actor frames;
...
- high-score defaults and name-entry rendering data;
...
round-trip or byte-accounting tests. Gameplay and rendering consume named
...
- complete local two-player replacement for link play, including independent
  devices, shared piece semantics, attacks, garbage, wins, draws and match progression.
...
- persistent Original, Enhanced, Modern and Custom profiles with schema migration;
- Original, Fast and Instant fixed-step line-clear pacing;
- exact original 160x144 rendering with integer scaling;
- native-resolution Widescreen Frame rendering;
...
- independently mixed native sound effects in Enhanced mode, with authentic
  Game Boy channel stealing retained by Original mode;
- separate Confirm/Back and clockwise/counterclockwise inputs, south-counterclockwise
  and east-clockwise face-button defaults, Tetris 99-style Up hard drop and L/R Hold,
  side rotation pushout, and adjustable 12/2-frame DAS/ARR in Enhanced mode;
- an opt-in Modern ruleset with hard and sonic drop, a landing ghost, hold,
  one-to-five previews, seven-bag and history randomizers, SRS wall/floor kicks,
  lock delay with move resets, instant autorepeat, spawn-input buffering,
  Guideline piece colors enabled by default, modern combo/back-to-back/T-spin scoring
  and quick restart;
- Type-A Marathon, 40-line Sprint and three-minute Ultra challenges;
...
Enhanced is the recommended classic preset; Modern is the opt-in contemporary
ruleset; Original remains a complete, tested compatibility preset. Pacing and
rules identities are stored in replay metadata when they affect outcomes.
...
palette-limited 160x144 target, or into a responsive extended canvas when the
extra columns fit at the same integer scale. Keep sharp gameplay pixels
authoritative while effects and backgrounds use separate layers.

The general compositor toybox and arbitrary non-Tetris variants remain outside
this bounded clean-port goal.

## Rendering

Use a resolution-independent rendering model with four coordinate spaces:

- board/layout coordinates;
- original 160x144 Game Boy composition coordinates;
- native logical rendering coordinates;
- final host pixels.
...
Render independently from the deterministic simulation step. High-refresh
...
Expose the two live Gubsy binding profiles through a direct ImGui Controls
window. Binding changes save immediately. Keep the player-facing UI free of
GBRE-specific memory editors and other reverse-engineering artifacts.
...
before entering the fixed-step simulation. Rising edges remain buffered until
the next game step consumes them. Render frequency must not alter DAS,
rotation, pause, menu edges or gameplay.

Pace rendering with an explicit hard 60/120/144/165/240 Hz cap independent of
display VSync. Use the GPU tile atlas by default and retain a selectable CPU
raster fallback. Optional active-piece interpolation consumes previous/current
fixed-step snapshots; interpolation off renders current state at 60 Hz. The
simulation accumulator determines when game steps run; it must never be used as
a reason to busy-spin or to reduce rendering to the Game Boy step rate.

Desktop hosts block while inactive. Browser hosts never enter that path: RAF
continues to sample input, C++ skips excess presentation work non-blockingly,
and JavaScript owns page-visibility suspension and elapsed-time reset.
...
- game screen, mode, score, level, timers and current semantic events;
...
- replay recording, playback, seed/state setup, pause and single-step stepping;
...
become runtime dependencies or force step-for-frame identity where semantic
...
4. Implement complete Type A, Type B, menus, high scores, demos and screen progression.
5. Implement complete local two-player semantics and device-independent match
   state.
6. Implement original rendering, every animation and every ending from the
   content catalogs.
...
- every original single-player, demo, ending and local two-player feature is
  implemented and reachable through ordinary play;
...
- the simulation contains no copied reference core, WRAM-shaped ownership,
  placeholder progression, stub behavior or unexplained magic state;
...
- manual playtesting finds no known blocker, missing content path, obvious
  rendering defect or obviously wrong audio.
