...
trap 'chmod -R u+w "$work_root"; rm -rf -- "$work_root"' EXIT

"$repo_root/scripts/audit-distribution.py"
...
"$checkout/scripts/audit-distribution.py" \
    --artifact "$build/native-gb-tetris-modern"
