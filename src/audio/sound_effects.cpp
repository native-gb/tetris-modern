...
void SoundEffects::attach(const content::AudioCatalog& source) {
    stop();
    catalog = &source;
}
...
bool SoundEffects::play(content::SoundChannel channel, std::uint8_t effect,
    ...
    // Resolve the original channel-local effect identifier.
    ...
    if (channel == content::SoundChannel::pulse && pulse_is_blocked(effect)) {
        events.push_back({SoundEventType::rejected, channel, effect});
        return false;
    }

    // Replace the channel with the effect's initial register snapshot.
    ...
    channels[index(channel)] = state;
    events.push_back({SoundEventType::started, channel, effect, 0, true});
    return true;
}

void SoundEffects::step(std::uint8_t random_sample) {
    events.clear();
...
const content::SoundEffect* SoundEffects::find(content::SoundChannel channel,
    ...
    if (catalog == nullptr)
        return nullptr;
    const auto found = std::ranges::find_if(catalog->effects, [channel, effect](const auto& entry) {
        return entry.channel == channel && entry.original_id == effect;
    });
    return found == catalog->effects.end() ? nullptr : &*found;
}

bool SoundEffects::pulse_is_blocked(std::uint8_t effect) const {
    // Reproduce the original priority rules for the shared pulse-effect channel.
...
void SoundEffects::advance(content::SoundChannel sound_channel, std::uint8_t random_sample) {
    SoundChannelState& state = channels[index(sound_channel)];
    if (!state.active())
        return;

    // Finish expired effects and chain the Tetris pulse into its wave sweep.
    ++state.elapsed;
    if (state.elapsed >= state.effect->duration) {
        const bool start_sweep =
            sound_channel == content::SoundChannel::pulse && state.effect->original_id == 7;
        stop(sound_channel);
        if (start_sweep)
    ...
    // The game-over effect rewrites its pitch from random samples each step.
    if (state.effect->random_pitch) {
        ...
        state.registers[3] =
            static_cast<std::uint8_t>((state.random_pitch & 0xF0U) | (random_sample & 0x0FU));
        events.push_back(
            {SoundEventType::changed, sound_channel, state.effect->original_id, state.elapsed});
        return;
    }

    // Apply every register script step scheduled for this step.
    for (const content::SoundStep& step : state.effect->steps) {
        if (step.at_step != state.elapsed)
            continue;
        ...
        events.push_back({SoundEventType::changed, sound_channel, state.effect->original_id,
                          state.elapsed, step.writes[4] && (step.registers[4] & 0x80U) != 0});
    }
}
...
void SoundEffects::stop(content::SoundChannel sound_channel) {
    ...
    if (!state.active())
        return;
    events.push_back(
        {SoundEventType::stopped, sound_channel, state.effect->original_id, state.elapsed});
    state = {};
}
