...
Provenance source_for(const Range& range, std::size_t expected_count, std::size_t decoded_count) {
    return provenance(range.id, {range.begin, range.end}, range.format, expected_count,
                      decoded_count);
}
...
constexpr std::array layout_ranges = {
...
// Decode the original joypad bit layout used by demo runs.
...
Tile decode_tile(std::span<const std::uint8_t> bytes, bool one_bit) {
    ...
    for (std::size_t y = 0; y < 8; ++y) {
        ...
        for (std::size_t x = 0; x < 8; ++x) {
            ...
            tile.pixels[y * 8 + x] =
                static_cast<std::uint8_t>(((low >> shift) & 1U) | (((high >> shift) & 1U) << 1U));
        }
    }
    return tile;
}

// Extract one contiguous 1bpp or 2bpp graphics bank.
...
bool extract_sprites(const Rom& rom, SpriteCatalog& result, std::string& error) {
    // The ROM reuses five coordinate matrices across every metasprite.
    struct Matrix {
        std::size_t begin;
        std::size_t count;
    };
    ...
    catalog.source =
        provenance("metasprite-catalog", {0x2B64, 0x323F}, "pointer-metasprites", 94, 94);

    // Follow each pointer chain and combine tile tokens with matrix coordinates.
    for (std::size_t id = 0; id < 94; ++id) {
        ...
        const auto matrix = std::ranges::find_if(
            matrices, [matrix_address](Matrix entry) { return entry.begin == matrix_address; });
        ...
        // Decode the tile list, including skip and horizontal-flip tokens.
        ...
        // Record every ROM span used by this logical sprite.
        sprite.source = provenance("metasprite-" + std::to_string(id),
...
bool extract_gameplay(const Rom& rom, const SpriteCatalog& sprites, GameplayCatalog& result,
                      std::string& error) {
    // Extract the per-level gravity timing table.
    ...
    gameplay.gravity_source =
        provenance("gravity-frames", gravity_span, "byte-per-level", 21, gravity.size());

    // Convert the first 28 metasprites into seven pieces with four rotations.
...
const Tilemap* Catalog::find_tilemap(std::string_view id) const {
    const auto found = std::ranges::find(
        tilemaps, id, [](const Tilemap& map) { return std::string_view(map.source.id); });
    return found == tilemaps.end() ? nullptr : &*found;
}

const ByteTable* Catalog::find_layout(std::string_view id) const {
    const auto found = std::ranges::find(layout_tables, id, [](const ByteTable& table) {
        return std::string_view(table.source.id);
    });
    return found == layout_tables.end() ? nullptr : &*found;
}
...
bool extract_catalog(const Rom& rom, Catalog& result, std::string& error) {
    ...
    // Extract the fixed graphics banks and recorded title demos.
    Catalog catalog;
    ...
    if (!extract_tiles(rom, tile_ranges[0], catalog.gameplay_tiles, error) ||
        ...
        !extract_demo(rom, {"type-a-demo", 0x62B0, 0x63B0, "joypad-rle"}, catalog.type_a_demo,
                      error) ||
        !extract_demo(rom, {"type-b-demo", 0x63B0, 0x6450, "joypad-rle"}, catalog.type_b_demo,
                      error))
        return false;

    // Decode the deterministic piece sequence used by both demos.
    ...
    catalog.demo_piece_source =
        provenance("demo-pieces", {0x6450, 0x6480}, "piece-codes", 48, catalog.demo_pieces.size());
    catalog.type_b_demo_garbage = {
        .source = provenance("type-b-demo-garbage", {0x1B40, 0x1B68}, "raw-board-cells", 40, 40),
        .bytes = {rom.bytes.begin() + 0x1B40, rom.bytes.begin() + 0x1B68},
    };

    // Copy small placement and text tables used directly by rendering.
    for (const Range& range : layout_ranges) {
        catalog.layout_tables.push_back({
    ...
    // Validate and copy every static screen tilemap.
    for (const MapRange& definition : tilemap_ranges) {
        ...
        if (definition.range.end - definition.range.begin !=
                cells + (definition.terminator ? 1U : 0U) ||
        ...
        catalog.tilemaps.push_back({
            ...
            .tiles = {rom.bytes.begin() + static_cast<std::ptrdiff_t>(definition.range.begin),
                      rom.bytes.begin() +
                          static_cast<std::ptrdiff_t>(definition.range.begin + cells)},
        });
    }

    // Decode linked sprites, gameplay tables, and audio last.
