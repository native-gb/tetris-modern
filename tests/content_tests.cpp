...
void expect_provenance(const tetris::content::Provenance& source, const tetris::content::Rom& rom) {
...
std::uint8_t encode_buttons(const tetris::Buttons& buttons) {
    return static_cast<std::uint8_t>((buttons.left ? 0x20U : 0U) | (buttons.right ? 0x10U : 0U) |
                                     (buttons.up ? 0x40U : 0U) | (buttons.down ? 0x80U : 0U) |
                                     (buttons.back ? 0x02U : 0U) |
                                     (buttons.confirm ? 0x01U : 0U) |
...
void expect_tile_round_trip(const tetris::content::TileSet& tiles,
    ...
    expect(std::equal(encoded.begin(), encoded.end(),
                      rom.bytes.begin() + static_cast<std::ptrdiff_t>(span.begin)),
           tiles.source.id + " round-trips to the source bytes");
}

void expect_demo_round_trip(const tetris::content::Demo& demo, const tetris::content::Rom& rom) {
...
void test_profile_validation_errors() {
    ...
    expect(!validate_supported(changed, error) &&
               error.find("header checksum") != std::string::npos,
           "bad header checksum is identified explicitly");
    changed = rom;
    changed.global_checksum_valid = false;
    expect(!validate_supported(changed, error) &&
               error.find("global checksum") != std::string::npos,
...
void test_supported_catalog() {
    ...
    expect(catalog.layout_tables.size() == 29, "all 29 placement and text tables are represented");
    ...
    const ByteTable* cursor = catalog.find_layout("music-cursor-coordinates");
    expect(cursor != nullptr && cursor->bytes == std::vector<std::uint8_t>{0x70, 0x37, 0x70, 0x77,
                                                                           0x80, 0x37, 0x80, 0x77},
    ...
    for (const Tilemap& map : catalog.tilemaps) {
        ...
        expect(std::equal(map.tiles.begin(), map.tiles.end(),
                          rom.bytes.begin() + static_cast<std::ptrdiff_t>(span.begin)),
               map.source.id + " tilemap cells round-trip");
    }
    for (const ByteTable& table : catalog.layout_tables) {
        expect_provenance(table.source, rom);
        const RomSpan span = table.source.spans.front();
        expect(std::equal(table.bytes.begin(), table.bytes.end(),
                          rom.bytes.begin() + static_cast<std::ptrdiff_t>(span.begin)),
    ...
    expect(std::equal(catalog.audio.source_bytes.begin(), catalog.audio.source_bytes.end(),
                      rom.bytes.begin() +
                          static_cast<std::ptrdiff_t>(catalog.audio.source.spans.front().begin)),
