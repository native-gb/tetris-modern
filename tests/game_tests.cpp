...
void test_button_buffer() {
    using namespace tetris;
    ButtonBuffer input;

    // A complete tap between game steps remains visible for exactly one step.
    sample_buttons(input, {.confirm = true});
    sample_buttons(input, {});
    expect(consume_buttons(input).confirm, "short button tap survives until the next step");
    expect(consume_buttons(input) == Buttons{}, "consumed button press does not repeat");

    // Held state remains visible for every step until a host sample releases it.
    sample_buttons(input, {.left = true});
    expect(consume_buttons(input).left, "new held direction reaches the first step");
    expect(consume_buttons(input).left, "held direction reaches catch-up steps");
    sample_buttons(input, {});
    expect(consume_buttons(input) == Buttons{}, "released direction clears held state");

    // Different edges accumulate when several host frames occur before a game step.
    sample_buttons(input, {.start = true});
    sample_buttons(input, {});
    sample_buttons(input, {.select = true});
    sample_buttons(input, {});
    const Buttons combined = consume_buttons(input);
    expect(combined.start && combined.select, "pending host-frame presses accumulate");
}
...
void step_until_falling(tetris::SinglePlayer& game) {
    int guard = 0;
...
void test_single_player_clear() {
    ...
    game.step({.buttons = {.down = true}, .random = {}});
    game.step({});
