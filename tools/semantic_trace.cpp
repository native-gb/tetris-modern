...
std::uint8_t key_bit(std::string_view name) {
    if (name == "A") return 1U << 0U;
    if (name == "B") return 1U << 1U;
    if (name == "SELECT") return 1U << 2U;
    if (name == "START") return 1U << 3U;
    if (name == "RIGHT") return 1U << 4U;
    if (name == "LEFT") return 1U << 5U;
    if (name == "UP") return 1U << 6U;
    if (name == "DOWN") return 1U << 7U;
    return 0;
}
...
bool load_input(const std::filesystem::path& path, std::vector<InputEvent>& events,
    ...
    while (std::getline(input, line)) {
        ...
        if (comma == std::string_view::npos ||
            !integer(content.substr(0, comma), event.frame) ||
...
Buttons buttons(std::uint8_t keys) {
    return {
        ...
        .up = (keys & (1U << 6U)) != 0,
        .down = (keys & (1U << 7U)) != 0,
        .rotate_left = (keys & (1U << 1U)) != 0,
        .rotate_right = (keys & (1U << 0U)) != 0,
        .start = (keys & (1U << 3U)) != 0,
        .select = (keys & (1U << 2U)) != 0,
    };
}
...
std::uint8_t sample(int frame, int salt) {
    const unsigned int value = static_cast<unsigned int>(frame) * 73U +
                               static_cast<unsigned int>(salt) * 151U;
...
bool load_setup(const std::filesystem::path& path, Setup& setup, std::string& error) {
    ...
    setup.apply_tick = json_integer(text, "apply_tick").value_or(0);
    ...
    if (setup.apply_tick < 0 || setup.rules.starting_level < 0 ||
        setup.rules.starting_level > 20 || setup.rules.type_b_height < 0 ||
        setup.rules.type_b_height > 5 || (setup.score && *setup.score > 999'999U)) {
...
void apply_setup(const Setup& setup, int frame, GameFlow& flow) {
    if (frame != setup.apply_tick)
        return;
    flow.start_session(setup.rules, startup_random(frame));
    if (setup.score)
        flow.edit_game().debug_set_score(*setup.score);
    if (setup.state)
        flow.edit_game().debug_set_state(*setup.state);
}
...
unsigned int piece_code(PieceSpec piece) {
    return static_cast<unsigned int>(piece.kind) * 4U +
...
unsigned int effect_id(const audio::Director& audio,
...
void write_snapshot(std::ofstream& output, int frame, std::uint8_t keys,
                    const GameFlow& flow, const audio::Director& audio) {
    const SinglePlayer& game = flow.game();
    const auto channels = audio.music().channels();
    output << "{\"kind\":\"frame\",\"frame\":" << frame
           << ",\"input_mask\":" << static_cast<unsigned int>(keys)
           << ",\"screen\":\"" << screen_name(flow.screen()) << "\""
           << ",\"ending_stage\":\"" << ending_name(flow.ending_stage()) << "\""
           << ",\"ending_elapsed\":" << flow.ending_elapsed()
           << ",\"launch_y\":" << (flow.launch_y() & 0xff)
           << ",\"exhaust_x\":" << flow.exhaust_x()
           << ",\"exhaust_y\":" << (flow.exhaust_y() & 0xff)
           << ",\"exhaust_frame\":" << flow.exhaust_animation_frame()
           << ",\"congratulations_characters\":" << flow.congratulations_characters()
           << ",\"timer\":" << flow.timer()
           << ",\"selected_mode\":" << static_cast<int>(flow.selected_type())
           << ",\"selected_music\":" << static_cast<int>(flow.selected_music())
           << ",\"selected_level\":" << flow.selected_level()
           << ",\"selected_height\":" << flow.selected_height()
           << ",\"heart_mode\":" << (flow.heart_mode() ? "true" : "false")
           << ",\"demo_type_b\":" << (flow.demo_is_type_b() ? "true" : "false")
           << ",\"phase\":\"" << phase_name(game.state()) << "\""
           << ",\"level\":" << game.level()
           << ",\"game_height\":" << game.rules().type_b_height
           << ",\"lines\":" << game.lines()
           << ",\"score\":" << game.score()
           << ",\"scoreboard_category\":" << flow.scoreboard().category
           << ",\"scoreboard_score\":" << flow.scoreboard().score
           << ",\"active\":[" << piece_code(game.piece()) << ','
           << game.piece().origin.x << ',' << game.piece().origin.y << ']'
           << ",\"preview\":" << piece_code(game.preview())
           << ",\"next_preview\":" << piece_code(game.hidden_piece())
           << ",\"preview_hidden\":" << (!game.preview_visible() ? "true" : "false")
           << ",\"drop_timer\":" << game.drop_timer()
           << ",\"fixed_pieces_consumed\":" << game.fixed_pieces_consumed()
           ...
           << ",\"music_base_periods\":[" << channels[0].period << ','
           << channels[1].period << ',' << channels[2].period << ']'
           << ",\"music_timers\":[" << static_cast<unsigned int>(channels[0].timer)
           << ',' << static_cast<unsigned int>(channels[1].timer) << ','
           << static_cast<unsigned int>(channels[2].timer) << ']'
           << ",\"board\":\"" << board_hex(game.board()) << "\"}\n";
}
...
int main(int argc, char** argv) {
    ...
    if (!parse_options(argc, argv, options)) {
        std::fprintf(stderr, "Usage: %s --rom file --input events.csv --output trace.jsonl "
    ...
    output << "{\"kind\":\"metadata\",\"schema\":\"native-gb-tetris.trace.v1\","
           << "\"rom_sha1\":\"" << content.source_sha1 << "\",\"frame_limit\":"
           << options.frames << "}\n";
    ...
    for (int frame = 0; frame < options.frames; ++frame) {
        ...
        if (setup)
            apply_setup(*setup, frame, flow);
        flow.tick({.player_one = buttons(held_keys),
                   .player_two = {},
                   .random = random_samples(frame, 300),
                   .startup = startup_random(frame),
                   .versus = versus_random(frame)});
        audio.tick(flow, static_cast<std::uint8_t>(flow.game().tick_count()));
        write_snapshot(output, frame, held_keys, flow, audio);
    }
    return output ? 0 : 1;
}
